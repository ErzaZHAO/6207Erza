...
[My GitHub Repository](https://github.com/ErzaZHAO/6207Erza)
...
## As a platform for temporary storage and file sharing, security, stability and privacy of Github are not that reliable. Files might be easily tampered with by others. There is also the risk of file data disappearing when the platform crashes.
...
# The result and image could only be shown without their codes.
```
...
write_csv(data, file = "output/data/erza.csv")
...
```
```{r, rmMiss}
fixed_data<- data %>% filter(!(is.na(activity)))
...
fixed_data %>% group_by(species,treatment) %>% summarise(mean(size))
...
## The red area shows the deleted codes and the green area shows new added codes.
```
...
```{r, prettyfig, fig.align='center', fig.cap="different_species"，echo=TRUE,eval = FALSE}
# ggplot figure showing mean activity for each treatment (Control, OA) for each species.
ggplot(fixed_data, aes(x = treatment, y = activity,color=treatment)) + geom_boxplot() + facet_wrap(~species)
...
library(png)
pic1 <- readPNG("./pics/acantho.png")
pic2 <- readPNG("./pics/ambon.png")
pic3 <- readPNG("./pics/chromis.png") 
pic4 <- readPNG("./pics/humbug.png")
pic5 <- readPNG("./pics/lemon.png")
pic6 <- readPNG("./pics/whitedams.png")
```
```{r}
library(ggplot2)
ggplot(fixed_data, aes(x = species, y = activity, color = treatment)) + geom_boxplot()  + annotation_raster(pic1, 0.5, 1.5, 5, 15, interpolate = FALSE) + annotation_raster(pic2, 1.5, 2.5, 45, 55, interpolate = FALSE) + annotation_raster(pic3, 2.5, 3.5, 46, 56, interpolate = FALSE) + annotation_raster(pic4, 3.5, 4.5, 5, 15, interpolate = FALSE) + annotation_raster(pic5, 4.5, 5.5, 3, 13, interpolate = FALSE) + annotation_raster(pic6, 5.5, 6.5, 3, 13, interpolate = FALSE)
...
ggsave(data, file = "output/figures/erza.jpg")
...
# acantho
data_acantho <- fixed_data %>% filter(species == 'acantho') 
data_a <- data_acantho %>% filter(treatment == "control")
data_a_test <- data_a %>% filter(treatment == "CO2")
test1 <- t.test(data_a$activity, data_a_test$activity)
print(test1)

# ambon
data_ambon<- fixed_data %>% filter(species == 'ambon') 
data_amb<- data_ambon %>% filter(treatment == "control")
data_amb_test <- data_amb %>% filter(treatment == "CO2")
test2 <- t.test(data_amb$activity, data_amb_test$activity)
print(test2)

#chromis
data_chromis <- fixed_data %>% filter(species == 'chromis') 
data_ch<- data_chromis %>% filter(treatment == "control")
data_ch_test <- data_ch %>% filter(treatment == "CO2")
test3 <- t.test(data_ch$activity, data_ch_test$activity)
print(test3)

#humbug
data_humbug <- fixed_data%>% filter(species == 'humbug') 
data_h<- data_humbug%>% filter(treatment == "control")
data_h_test <- data_h %>% filter(treatment == "CO2")
test4 <- t.test(data_h$activity, data_h_test$activity)
print(test4)

#lemon
data_lemon<- fixed_data %>% filter(species == 'lemon') 
data_l <- data_lemon %>% filter(treatment == "control")
data_l_test<- data_l %>% filter(treatment == "CO2")
test5 <- t.test(data_l$activity, data_l_test$activity)
print(test5)

#whitedams
data_whitedams <- fixed_data %>% filter(species == 'whitedams') 
data_w <- data_whitedams%>% filter(treatment == "control")
data_w-test <- data_w%>% filter(treatment == "CO2")
test6 <- t.test(data_w$activity, data_whitedams$activity)
print(test6)
